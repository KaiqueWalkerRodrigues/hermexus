...
{
    public class WhatsappContactEnricher : ContentResponseEnricher<WhatsappContactDTO>
    {
        protected override Task EnrichModel(
            WhatsappContactDTO content, IUrlHelper urlHelper
            )
        {
            var requet = urlHelper.ActionContext.HttpContext.Request;
            var baseUrl = $"{requet.Scheme}://{requet.Host.ToUriComponent()}{requet.PathBase.ToUriComponent()}/api/whatsapp_contact/v1";
