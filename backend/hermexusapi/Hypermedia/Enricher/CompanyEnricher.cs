...
{
    public class CompanyEnricher : ContentResponseEnricher<CompanyDTO>
    {
        protected override Task EnrichModel(
            CompanyDTO content, IUrlHelper urlHelper
            )
        {
            var requet = urlHelper.ActionContext.HttpContext.Request;
            var baseUrl = $"{requet.Scheme}://{requet.Host.ToUriComponent()}{requet.PathBase.ToUriComponent()}/api/company/v1";
