...
{
    public class RobotEnricher : ContentResponseEnricher<RobotDTO>
    {
        protected override Task EnrichModel(
            RobotDTO content, IUrlHelper urlHelper
            )
        {
            var requet = urlHelper.ActionContext.HttpContext.Request;
            var baseUrl = $"{requet.Scheme}://{requet.Host.ToUriComponent()}{requet.PathBase.ToUriComponent()}/api/robot/v1";
