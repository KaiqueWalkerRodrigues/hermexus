...
{
    public class PermissionEnricher : ContentResponseEnricher<PermissionDTO>
    {
        protected override Task EnrichModel(
            PermissionDTO content, IUrlHelper urlHelper
            )
        {
            var requet = urlHelper.ActionContext.HttpContext.Request;
            var baseUrl = $"{requet.Scheme}://{requet.Host.ToUriComponent()}{requet.PathBase.ToUriComponent()}/api/permission/v1";
